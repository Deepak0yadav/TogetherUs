...
export default function GameHUD({
  spaceCode, roomName, onCopyInvite, onChangeSpace,
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <span className="pixel-font" style={{ fontSize: 8, color: '#c4c4d4', letterSpacing: '1px' }}>
            {roomName || 'Your Space'}
          </span>
        </div>
        ...
        {spaceCode && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <input readOnly value={spaceCode} style={{
              background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)',
              borderRadius: 8, color: '#e8e8f0', fontSize: 12, fontWeight: 600,
              padding: '5px 12px', width: 140, textAlign: 'center', outline: 'none',
              fontFamily: 'Inter, monospace', letterSpacing: '1px',
            }} />
            <button onClick={onCopyInvite} id="copy-invite-btn" style={{
      ...
      {showSwitchModal && (
        <div style={{
          ...
          <div className="glass-card purple-glow" style={{ width: 380, padding: 30, textAlign: 'center' }}>
            ...
            <input
              autoFocus type="text" placeholder="e.g. 74424b7f7d37"
              value={joinCodeInput}
              ...
              style={{
                ...
                padding: '12px 16px', color: '#e8e8f0', fontSize: 14, outline: 'none',
                fontFamily: 'Inter, monospace', textAlign: 'center', marginBottom: 20,
              }}
            />
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowSwitchModal(false)} style={{
                ...
                padding: '10px 0', cursor: 'pointer', fontWeight: 500,
              }}>Cancel</button>
              <button onClick={handleSwitchSubmit} disabled={!joinCodeInput.trim()} style={{
                flex: 1,
                background: joinCodeInput.trim() ? '#7c3aed' : 'rgba(124,58,237,0.4)',
      ...
      <div style={{
        ...
        <HudButton id="hud-mic" icon={micOn ? '🎤' : '🔇'}
          label={micOn ? 'Mute mic' : 'Unmute mic'} active={micOn} danger={!micOn}
          onClick={onToggleMic} />
        <HudButton id="hud-cam" icon={camOn ? '📷' : '🚫'}
          label={camOn ? 'Turn off camera' : 'Turn on camera'} active={camOn} danger={!camOn}
          onClick={onToggleCam} />

        <div style={{ width: 1, height: 32, background: 'rgba(255,255,255,0.1)', margin: '0 4px' }} />

        <HudButton id="hud-chat" icon="💬" label="Chat" active={chatOpen} onClick={onToggleChat} />
        <HudButton id="hud-emoji" icon="👋" label="React" onClick={onToggleEmoji} />
        <HudButton id="hud-share" icon="🖥️" label="Share screen" onClick={onToggleScreenShare} />

        <div style={{ width: 1, height: 32, background: 'rgba(255,255,255,0.1)', margin: '0 4px' }} />

        <HudButton id="hud-fullscreen" icon={isFullscreen ? '⬜' : '⛶'}
          label={isFullscreen ? 'Exit fullscreen' : 'Fullscreen'} onClick={toggleFullscreen} />
        <HudButton id="hud-settings" icon="⚙️" label="Settings" onClick={onOpenSettings} />
        ...
        <div style={{
          ...
          borderRadius: 12, padding: '6px 14px', cursor: 'default',
        }}>
          <div style={{
          ...
          <button onClick={() => useAuthStore.getState().logout()} title="Leave & logout" style={{
